...
This repo provides an example project for the [dbt-spark-cde](https://github.com/cloudera/dbt-spark-cde) adapter for [dbt](https://www.getdbt.com/).
...
`git clone https://github.com/cloudera/dbt-spark-cde-example.git`
...
Copy these two files to the seeds folder inside `dbt_spark_cde_demo` dbt project directory.
...
Move to the `dbt_spark_cde_demo` dbt project directory.
...
We also have a custom test created in `dbt_spark_cde_demo/tests/generic/test_length.sql` which is used to test the character length of a column.
